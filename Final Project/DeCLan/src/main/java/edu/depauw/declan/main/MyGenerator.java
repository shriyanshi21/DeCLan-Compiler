...
public class MyGenerator implements Generator {
	...
	private Value evalConstUnOp(UnaryOperation expr) {
		Value operand = evalConst(expr.getExpression());
		switch (expr.getOperator()) {
		case PLUS:
			if (operand instanceof IntValue) {
				IntValue o = (IntValue) operand;
				return new IntValue(o.getValue());
			}
			else {
				RealValue o = (RealValue) operand;
				return new RealValue(o.getValue());
			}
		case MINUS:
			if (operand instanceof IntValue) {
				IntValue o = (IntValue) operand;
				return new IntValue(o.getValue()*-1);
			}
			else {
				RealValue o = (RealValue) operand;
				return new RealValue(o.getValue()*-1);
			}
		case NOT:
			BoolValue o = (BoolValue) operand;
			return new BoolValue(!(o.getValue()));
	...
	private void generateRepeatStatement(RepeatStatement stmt, List<ICode> result) {
		...
		String ifFalse = newLabel();
		String end = newLabel();
		generateBoolean(stmt.getTest(), ifTrue, ifFalse, result);
		result.add(new Label(ifTrue));
		result.add(new Label(end));		
		result.add(new Label(ifFalse));
		result.add(new Goto(loop));
	...
	private void generateBooleanUnOp(UnaryOperation expr, String ifTrue, String ifFalse, List<ICode> result) {
		...
		generateBoolean(expr.getExpression(), ifFalse, ifTrue, result);
	...
	private void generateExprUnOp(UnaryOperation expr, Type.ExprType type, String place, List<ICode> result) {
		...
		generate(expr.getExpression(), temp, result);

		
		switch (expr.getOperator()) {
		case PLUS:
				result.add(new LetVar(place, temp));
				break;
		case MINUS:
			if (isInt) {
				result.add(new LetUn(place, LetUn.Op.INEG, temp));
			} else {
				result.add(new LetUn(place, LetUn.Op.RNEG, temp));
			}
			break;
		    default:
